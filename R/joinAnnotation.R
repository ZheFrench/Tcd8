...
option_list = list(
  ...
  make_option(c("-e", "--expression"), type="character",  default = glue("{base.dir}NanoString.normalised.tsv"),help="File with IDs and Annotations", metavar="PATH2ANNOTATION"),
      make_option(c("-n", "--name"), type="character", default=as.character("default"),  help="Add name to file", metavar="ADDNAME")
...
colnames(dataframe.final)[3] <- "item"

# Subset what you want to keep
dataframe.final <- dataframe.final %>% select("Patient","Best.Response","Responder.1","Responder.2","Histo","Tabaco" ,"Nb.Pa", "Sexe","Age.First.Immunotherapy","group","Outlier","item") 
...
dataframe.nanostring.transposed.annotated <- dataframe.nanostring.transposed.annotated %>% select(c("Patient","Best.Response","Responder.1","Responder.2","Histo","Tabaco" ,"Nb.Pa", "Sexe","Age.First.Immunotherapy","group","Outlier","item") , everything())

dataframe.nanostring.annotated <- transpose(dataframe.nanostring.transposed.annotated)

rownames(dataframe.nanostring.annotated) <- c (c("Patient","Best.Response","Responder.1","Responder.2","Histo","Tabaco" ,"Nb.Pa", "Sexe","Age.First.Immunotherapy","group" ,"Outlier","item") , Genes)
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),], aes(factor(group),Age.First.Immunotherapy, fill=factor(group))) + 
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),] )+ geom_bar( aes( factor(group),fill = factor(Best.Response)) ,color="black")  + 
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),], aes(factor(group),Nb.Pa, fill=factor(group))) + geom_boxplot() + 
stat_compare_means(label="p.signif",method = "wilcox.test", paired = FALSE,label.x = 1.5) + geom_boxplot(outlier.shape=NA) + #label.x = 2.45
labs(x = "",fill = "Group :",y = "")  + geom_jitter( position=position_jitter(0.2))+ 
scale_fill_manual(values= c( "low"= "#00BFC4", "high"= "#F8766D"))+ theme(legend.position="right"   ,axis.text.y = element_text(size=14)) + theme_ipsum() )
dev.off()
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),] )+ geom_bar( aes( factor(group),fill = factor(Histo)),color="black"   )   + 
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),] )+ geom_bar( aes( factor(group) ,fill = factor(Sexe)) ,color="black" )  + 
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),] )+ geom_bar( aes( factor(group),fill = factor(Tabaco)) ,color="black")  + 
...
print(ggplot(dataframe.final[!is.na(dataframe.final$group),], aes(factor(group),TIS_Score, fill=factor(group))) +
stat_compare_means(label="p.signif",method = "wilcox.test", paired = FALSE,label.x = 1.5) + geom_boxplot(outlier.shape=NA) + #label.x = 2.45
labs(x = "",fill = "Group :",y = "")  + geom_jitter( position=position_jitter(0.2))+ 
scale_fill_manual(values= c( "low"= "#00BFC4", "high"= "#F8766D"))+ theme(legend.position="right"   ,axis.text.y = element_text(size=14)) + theme_ipsum() )
dev.off()
